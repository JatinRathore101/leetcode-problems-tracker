...
// One table row. This is the table's only client component: it owns both the
// copy button (Problem cell) and the status chip that opens the update form
// (Status cell), so the two cells have to share a single piece of state.
    ...
    <tr>
      ...
      <td>
        <span className="problem-cell">
          <a
            href={problem.link}
            className={isLocked ? 'problem-link--locked' : undefined}
            target="_blank"
            rel="noreferrer"
          >
            {problem.name}
      ...
      <td className="col-center">
        {/* The chip is the update trigger. A LOCKED row gets a plain,
            unclickable chip: the form has no LOCKED option to prefill. */}
        <Chip
          text={problem.status}
          state={problem.status}
          {...(isLocked
            ? { title: 'Locked — LeetCode Premium required' }
            : { title: 'Update problem', onClick: openUpdate })}
        />
